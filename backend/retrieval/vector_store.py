...
class VectorStoreService:
    ...
    def search(
        ...
        # Convert allowed document IDs into a set
        valid_doc_set = (
            set(int(x) for x in user_doc_ids)
        ...
        # If filtering is being used, Chroma will only search
        # inside the user's documents.
        if valid_doc_set:
            query_kwargs = {
                "query_embeddings": [query_embedding],
                "n_results": min(n_search, len(valid_doc_set)),
                "where": {
                    "document_id": {
                        "$in": list(valid_doc_set)
                    }
                },
                "include": [
                    "documents",
        ...
        if results and results.get("ids") and results["ids"][0]:
            ...
            for chunk_id, doc_text, meta, dist in zip(
                ...
                # Extra safety check
                if (
                    valid_doc_set is not None
                    and doc_id not in valid_doc_set
                ):
                    continue
        ...
        # Interleave candidate chunks from different documents
        retrieved_items = []
        ...
        for doc_id in doc_ids:
            retrieved_items.extend(
                retrieved_by_doc[doc_id][:max_chunks_per_doc]
            )
    ...
    def delete_document_chunks(
        ...
        try:
            ...
            print(
                f"[VectorStoreService] "
                f"Error deleting chunks for document "
                f"{document_id}: {e}"
            )
