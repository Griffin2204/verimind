...
from memory.memory_service import (
    get_validated_memories,
    create_memory,
    confirm_memory,
    delete_memory
)

from memory.conflict_service import (
...
async def upload_document(
    ...
    try:
        ...
        parsed_sections = parse_document(
            filename,
            content
        )
        ...
        raise HTTPException(
            ...
            detail={
                "error": {
                    "code": "PARSE_ERROR",
                    "message": (
                        f"Could not parse file content: {str(e)}"
                    )
                }
            }
        )
...
# ============================================================
# GET /api/v1/documents?user_id=1
# ============================================================
...
def submit_feedback(
        ...
        raise HTTPException(
            ...
            detail={
                "error": {
                    "code": "INVALID_LABEL",
                    "message": (
                        f"Label must be one of {allowed_labels}"
                    )
                }
            }
        )
    ...
    # If feedback is an explicit correction,
    # add as a candidate memory with conflict check
...
def resolve_conflict_endpoint(
    ...
    try:
        ...
        success, conflict = resolve_conflict(
            db,
            conflict_id,
            payload.resolution
        )
